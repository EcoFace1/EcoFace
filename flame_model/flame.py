...
FLAME_MESH_PATH = "./flame_model/assets/head_template_mesh.obj"
FLAME_LMK_PATH = "./flame_model/assets/landmark_embedding_with_eyes.npy"

# to be downloaded from https://flame.is.tue.mpg.de/download.php
FLAME_MODEL_PATH = "./flame_model/assets/generic_model.pkl"  # FLAME 2020
#FLAME_MODEL_PATH = "flame_model/assets/flame/flame2023.pkl"  # FLAME 2023 (versions w/ jaw rotation)
FLAME_PARTS_PATH = "./flame_model/assets/FLAME_masks.pkl" # FLAME Vertex Masks
...
def face_vertices(vertices, faces):
    ...
    :return: [batch size, number of fac es, 3, 3]
    """
...
class FlameHead(nn.Module):
    ...
    def forward(
        ...
        """
        Input:
            expression_params: N X time X number of expression parameters
            pose_params: N X time X number of pose parameters 
        return:d
            vertices: N X time X V X 3
            landmarks: N X time X number of landmarks X 3
        """
        device=expr.device
        #ret_vals = torch.zeros(expr.shape[0], expr.shape[1], 5143, 3).to(device)
        ret_vals = torch.zeros(expr.shape[0], expr.shape[1], 5023, 3).to(device)  
        ldmark = torch.zeros(expr.shape[0], expr.shape[1], 70, 3).to(device)
        batch_size = expr.shape[0]
        for i in range(batch_size):
            time_size = expr[i].shape[0]
            
            betas = torch.cat([shape[i], expr[i]], dim=1)
            #betas = expr[i]
            full_pose = torch.cat([rotation[i], neck[i], jaw[i], eyes[i]], dim=1)
            
            template_vertices = self.v_template.unsqueeze(0).expand(time_size, -1, -1)
...
if __name__ == '__main__':
    flame_model = FlameHead(shape_params=100, expr_params=50)
